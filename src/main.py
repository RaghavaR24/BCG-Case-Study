...
class AnalysisApp:
    ...
    def load_data(self):
        charges = self.spark.read.csv("Data/Charges_use.csv", header=True, inferSchema=True)
        endorsements = self.spark.read.csv("Data/Endorse_use.csv", header=True, inferSchema=True)
        restrict = self.spark.read.csv("Data/Restrict_use.csv", header=True, inferSchema=True)
        damages = self.spark.read.csv("Data/Damages_use.csv", header=True, inferSchema=True)
        primary_person = self.spark.read.csv("Data/Primary_Person_use.csv", header=True, inferSchema=True)
        unit = self.spark.read.csv("Data/Units_use.csv", header=True, inferSchema=True)
    ...
    def analysis_10(self):
        ...
        result = self.data["unit"].join(
            ...
            (col("VEH_COLOR_ID").isin(["BGE", "BLK", "BLU", "BRO",
                         "BRZ", "CAM", "CPR", "GLD", "GRY", "MAR"])) 
        ).groupBy("VEH_MAKE_ID").count().orderBy(col("count").desc()).limit(5)
